...
import type { DevToolsEvent, CircuitInfo } from "../types"
...
export function CircuitDashboard({ initialEvents, initialCircuits }: Props) {
  ...
  const merged = useMemo(() => {
    const map = new Map<
      string,
      { key: string; service: string; method: string; state: string; failures: number; lastTransitionAt: number; lastError?: string }
    >()
    ...
    <>
      <section className="nv-grid" style={{ marginBottom: 16 }}>
        <div className="nv-card">
          ...
          <div className="nv-value">{counts.total}</div>
        </div>
        <div className="nv-card">
          <h2>Open</h2>
        ...
        <div className="nv-card">
          <h2>Half-open</h2>
          <div className="nv-value" style={{ color: "var(--warn)" }}>
            {counts.half}
          </div>
        </div>
        <div className="nv-card">
          <h2>Closed</h2>
          <div className="nv-value" style={{ color: "var(--ok)" }}>
            {counts.closed}
          </div>
        </div>
      </section>
              ...
              {merged.map((c) => (
                <tr key={c.key}>
                  <td className="nv-mono">
                    <a href={`/services/${encodeURIComponent(c.service)}`}>{c.service}</a>
                  </td>
                  <td className="nv-mono">{c.method}</td>
                  <td>
                    {c.state === "closed" ? (
                      <span className="nv-badge ok">closed</span>
                    ) : c.state === "half-open" ? (
                      <span className="nv-badge warn">half-open</span>
                    ) : (
                      <span className="nv-badge err">open</span>
                    )}
                  </td>
                  <td>{c.failures}</td>
                  <td className="nv-muted" suppressHydrationWarning>
                    {c.lastTransitionAt ? formatRelative(c.lastTransitionAt) : "—"}
                  </td>
