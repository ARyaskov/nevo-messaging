...
export function MethodsLeaderboard({ initialEvents, highlightService, highlightMethod }: Props) {
  ...
  const highlightKey = highlightService && highlightMethod ? `${highlightService}:${highlightMethod}` : null
  ...
  return (
    <section className="nv-card">
      ...
      <p className="nv-muted" style={{ marginBottom: 12 }}>
        {help}
      </p>
        ...
        <table className="nv-table">
          ...
          <tbody>
            {data.map((s, i) => {
              ...
              return (
                <tr key={key} ref={isHighlight ? highlightedRowRef : null} className={isHighlight ? "nv-highlight" : undefined}>
