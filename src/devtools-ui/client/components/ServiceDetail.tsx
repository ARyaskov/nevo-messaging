...
import type { DevToolsEvent, ServiceInfo, CircuitInfo } from "../types"
    ...
    <>
      ...
      <section className="nv-grid" style={{ marginBottom: 16 }}>
        <div className="nv-card">
          <h2>Calls</h2>
          <div className="nv-value">{totals.total}</div>
        </div>
        <div className="nv-card">
          <h2>Errors</h2>
        ...
        <div className="nv-card">
          <h2>~RPS (60s)</h2>
          <div className="nv-value">{totals.rps.toFixed(2)}</div>
        </div>
        <div className="nv-card">
          <h2>Avg p95</h2>
          <div className="nv-value">{formatDuration(totals.avgP95)}</div>
        </div>
        <div className="nv-card">
          <h2>Methods declared</h2>
          <div className="nv-value">{initialServiceInfo?.methods.length ?? "—"}</div>
        </div>
      </section>
            ...
            <tbody>
              {initialCircuits.map((c) => (
                <tr key={c.key}>
                  ...
                  <td>{c.failures}</td>
                  <td className="nv-muted" suppressHydrationWarning>
                    {formatRelative(c.lastTransitionAt)}
                  </td>
                </tr>
              ))}
            </tbody>
