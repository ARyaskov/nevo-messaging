    ...
    <div>
      <section className="nv-grid" style={{ marginBottom: 24 }}>
        <div className="nv-card">
          <h2>Total events</h2>
          <div className="nv-value">{overview.totalEvents}</div>
        </div>
        <div className="nv-card">
          <h2>Errors</h2>
          <div className="nv-value" style={{ color: "var(--err)" }}>
            {overview.totalErrors}
          </div>
        </div>
        <div className="nv-card">
          <h2>Err rate</h2>
          <div className="nv-value">{formatErrorRate(overview.errorRate)}</div>
        </div>
        <div className="nv-card">
          <h2>Services</h2>
        ...
        <div className="nv-card">
          <h2>Methods</h2>
          <div className="nv-value">
            <a href="/methods">{overview.methods}</a>
          </div>
        </div>
        <div className="nv-card">
          <h2>~RPS (60s)</h2>
          <div className="nv-value">{overview.rps.toFixed(2)}</div>
        </div>
      </section>
      ...
      <section className="nv-grid" style={{ marginBottom: 24 }}>
        <div className="nv-card" style={{ gridColumn: "span 2" }}>
          ...
          {slowest.length === 0 ? (
            <div className="nv-empty">No data</div>
          ) : (
              ...
              <tbody>
                {slowest.map((s) => (
                  <tr key={`${s.service}:${s.method}`}>
                    ...
                    <td className="nv-mono">
                      <a href={methodLink(s.service, s.method)}>{s.method}</a>
                    </td>
        ...
        <div className="nv-card" style={{ gridColumn: "span 2" }}>
          ...
          {mostErrors.length === 0 ? (
            <div className="nv-empty">No errors recorded</div>
          ) : (
            <table className="nv-table">
              <thead>
                <tr>
        ...
        {recent.length === 0 ? (
          ...
          <table className="nv-table">
            ...
            <tbody>
              {recent.map((e, i) => (
                <tr key={`${e.ts}-${i}`}>
                  <td className="nv-mono nv-muted" suppressHydrationWarning>
                  ...
                  <td className="nv-mono">{e.method ? <a href={methodLink(e.service, e.method)}>{e.method}</a> : "-"}</td>
                  <td>
                    {e.status === "ok" ? (
                      <span className="nv-badge ok">ok</span>
                    ) : e.status === "error" ? (
                      <span className="nv-badge err">error</span>
                    ) : (
                      "-"
                    )}
                  </td>
                  ...
                  <td className="nv-mono nv-muted" style={{ fontSize: 11 }}>
                    {e.chainId ? (
                      <a href={`/traces?chain=${encodeURIComponent(e.chainId)}`} title={e.chainId}>
                        {e.chainId.slice(0, 8)}…
                      </a>
                    ) : (
                      ""
