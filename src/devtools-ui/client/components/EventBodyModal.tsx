...
export function EventBodyModal({ event, onClose }: Props) {
  ...
  return (
    <div className="nv-modal-backdrop" role="dialog" aria-modal="true" aria-label="Event body" onClick={onClose}>
      <div className="nv-modal" onClick={(e) => e.stopPropagation()}>
        <div className="nv-modal-header">
          ...
          <div className="nv-row" style={{ gap: 8 }}>
            <button type="button" className="nv-btn" onClick={handleCopy} aria-live="polite">
              {copied ? "Copied!" : "Copy"}
            </button>
            <button type="button" className="nv-btn-icon" onClick={onClose} aria-label="Close" title="Close (Esc)">
              ×
            </button>
          </div>
        </div>
