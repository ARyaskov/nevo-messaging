...
import type { ServiceInfo, AccessRule, AccessControlConfig } from "../types"
...
function isAccessAllowed(config: AccessControlConfig | undefined, topic: string, method: string, callerService: string | undefined): boolean {
  ...
  for (const rule of candidates) {
    matched = true
    if (listHasAcl(rule.deny, callerService)) {
      denied = true
      continue
    }
      ...
      {without.length > 0 && (
        <section className="nv-card" style={{ marginBottom: 16 }}>
          ...
          <div className="nv-row" style={{ flexWrap: "wrap", gap: 8 }}>
            {without.map((s) => (
              <span key={s.serviceName} className="nv-mono" style={{ padding: "4px 8px", border: "1px solid var(--border)", borderRadius: 6 }}>
                {s.serviceName}
              </span>
            ))}
          </div>
      ...
      {service && config && (
        <section className="nv-card">
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            {decision?.allowed ? <span className="nv-badge ok">ALLOW</span> : <span className="nv-badge err">DENY</span>}
          </div>
...
function Field({ label, value, onChange, placeholder }: { label: string; value: string; onChange: (v: string) => void; placeholder?: string }) {
  return (
    <label style={{ display: "flex", flexDirection: "column", gap: 4, minWidth: 180 }}>
      <span className="nv-muted" style={{ fontSize: 12 }}>
        {label}
      </span>
      <input
        value={value}
