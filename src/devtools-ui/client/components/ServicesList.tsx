...
import type { DevToolsEvent, ServiceInfo } from "../types"
import { useEvents } from "../lib/use-events"
import { computeServiceStats, formatErrorRate, formatRelative } from "../lib/computations"
  ...
  if (rows.length === 0) {
    return (
      <div className="nv-empty">
        No services registered yet. Run a service that uses NatsSignalRouter / KafkaSignalRouter / HttpSignalRouter / SocketSignalRouter.
      </div>
    )
  }
        ...
        {rows.map(([name, { info, stats }]) => (
          <tr key={name}>
            <td className="nv-mono">
              <a href={`/services/${encodeURIComponent(name)}`}>{name}</a>
            </td>
            ...
            <td className="nv-muted" suppressHydrationWarning>
              {stats?.lastTs ? formatRelative(stats.lastTs) : "—"}
            </td>
            <td>
              {info?.accessControl?.rules?.length ? (
                <span className="nv-badge warn">{info.accessControl.rules.length} rules</span>
              ) : (
                <span className="nv-muted">none</span>
              )}
            </td>
            <td>
              <a className="nv-mono" href={`/services/${encodeURIComponent(name)}`}>
                →
              </a>
            </td>
          </tr>
