...
export class NevoHttp2Client {
  ...
  private async createBody(method: string, params: unknown, type: MessageType, opts?: any) {
    const uuid = uuidv7()
    ...
    const compressed = this.compression.async ? await maybeCompressAsync(raw, this.compression) : maybeCompress(raw, this.compression)
  ...
  private async getSession(serviceName: string): Promise<SessionEntry> {
    ...
    session.on("error", (err) => {
      this.logger.warn({ event: "http2.session.error", err: err.message })
    })
    return entry
  }
        ...
        const result = await withRetry(async (attempt) => {
          ...
          try {
            ...
            const timer = setTimeout(() => {
              ...
              reject(new TimeoutError(serviceName, method, opts?.timeoutMs ?? this.timeoutMs))
            ...
            stream.on("end", () => {
              ...
              void (async () => {
                try {
                  ...
                  const decompressed = await maybeDecompressAsync(respBuf, respEncoding, this.maxPayloadBytes)
                  ...
                  reject(
                    err instanceof MessagingError ? err : new MessagingError(ErrorCode.PARSE_ERROR, { message: err?.message ?? "decode failed" })
                  )
                }
              })()
            })
            stream.on("error", (err: any) => {
              ...
              reject(
                err instanceof MessagingError
                  ? err
                  : new MessagingError(ErrorCode.CONNECTION_LOST, { message: err?.message ?? "stream error" }, serviceName)
              )
            })
            stream.end(buf)
            ...
            if (attempt > 1)
              this.metrics.incCounter(NEVO_METRIC_NAMES.retries, { transport: "http2", service: serviceName, method: methodLabel(method) })
          }
        }, this.retryOptions)
