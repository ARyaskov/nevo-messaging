...
test("withRetry succeeds without retries", async () => {
  ...
  const r = await withRetry(
    ...
    resolveRetryOptions({ maxAttempts: 3 })
  )
  assert.equal(r, "ok")
  assert.equal(n, 1)
})
...
test("explicit retryable false overrides an intrinsically retryable code", () => {
  const opts = resolveRetryOptions({ maxAttempts: 3 })
  const err = new MessagingError(ErrorCode.SERVICE_UNAVAILABLE, { retryable: false })
  assert.equal(shouldRetry(err, opts), false)
})
